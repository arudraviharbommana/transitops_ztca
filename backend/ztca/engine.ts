...
const ROLE_ACCESS_MATRIX: Record<string, ZTCARequestContext['requiredPrivilege'][]> = {
  Admin: ['READ_OPERATIONS', 'DISPATCH_TRIP', 'MANAGE_EXPENSES', 'ADMIN_POLICIES', 'SUPER_USER'],
  'Fleet Manager': ['READ_OPERATIONS', 'WRITE_VEHICLES', 'WRITE_DRIVERS', 'DISPATCH_TRIP'],
  Driver: ['READ_OPERATIONS', 'DISPATCH_TRIP', 'MANAGE_EXPENSES'],
  'Safety Officer': ['READ_OPERATIONS', 'WRITE_DRIVERS', 'DISPATCH_TRIP'],
  'Financial Analyst': ['READ_OPERATIONS', 'MANAGE_EXPENSES'],
  System: ['READ_OPERATIONS', 'ADMIN_POLICIES']
};

const isRoleAuthorized = (userRole: ZTCARequestContext['userRole'], requiredPrivilege: ZTCARequestContext['requiredPrivilege']) => {
  const allowedPrivileges = ROLE_ACCESS_MATRIX[userRole] ?? ['READ_OPERATIONS'];
  return allowedPrivileges.includes(requiredPrivilege);
};
...
export class ZTCAEngine {
  ...
  public makeDecision(context: ZTCARequestContext, risk: RiskEvaluation): ZTCADecision {
    ...
    // MANDATORY RULE 2: Least-Privilege Role Matrix
    if (!isRoleAuthorized(context.userRole, context.requiredPrivilege)) {
      return {
        outcome: 'BLOCK',
        riskScore: risk.totalScore,
        riskLevel: risk.level,
        reason: `ZTCA Security Policy Violation: Role '${context.userRole}' is not permitted to perform '${context.requiredPrivilege}' on '${context.endpoint}'.`,
        policyTriggered: 'pol_6_role_matrix',
        riskFactors: risk.factors,
        timestamp
      };
    }

    // MANDATORY RULE 3: Session Revocation
    if (context.sessionRevoked) {
      return {
        outcome: 'BLOCK',
        riskScore: risk.totalScore,
        riskLevel: risk.level,
        reason: 'ZTCA Security Policy Violation: This session has been revoked and can no longer access TransitOps resources.',
        policyTriggered: 'pol_7_session_revocation',
        riskFactors: risk.factors,
        timestamp
      };
    }
