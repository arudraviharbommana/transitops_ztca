...
export function ztcaAuthorizationMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  const sessionRevokedHeader = (req.headers['x-ztca-session-revoked'] as string | undefined)?.toLowerCase();
  const sessionStateHeader = (req.headers['x-ztca-session-state'] as string | undefined)?.toLowerCase();
  const sessionRevoked = sessionRevokedHeader === 'true' || sessionRevokedHeader === '1' || sessionStateHeader === 'revoked';
