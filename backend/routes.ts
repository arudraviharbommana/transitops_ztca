...
router.post('/auth/register', (req: Request, res: Response) => {
  ...
  if (role === 'Driver') {
    res.status(403).json({ error: 'Drivers must be added by a Fleet Manager through the driver management workflow.' });
    return;
  }
...
router.post('/vehicles', (req: Request, res: Response) => {
  const actorRole = (req.headers['x-ztca-user-role'] as string | undefined)?.trim();
  if (actorRole !== 'Fleet Manager') {
    res.status(403).json({ error: 'Only Fleet Managers can add vehicles.' });
    return;
  }
...
router.post('/drivers', (req: Request, res: Response) => {
  const actorRole = (req.headers['x-ztca-user-role'] as string | undefined)?.trim();
  if (actorRole !== 'Fleet Manager') {
    res.status(403).json({ error: 'Only Fleet Managers can add drivers.' });
    return;
  }
  ...
  const loginEmail = normalizedEmail || `${name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '.').replace(/\.$/, '')}-${Date.now()}@transitops.local`;

  const existingUser = userRepo.getByEmail(loginEmail);
  if (existingUser) {
    res.status(400).json({ error: `A user with email '${loginEmail}' already exists.` });
    return;
  }
  ...
  activityRepo.create(`Registered driver ${name} (${finalLicenseNumber}) and created login account for ${loginEmail}.`, userContext || 'Fleet Manager');
  auditDriverLicenses();
  res.status(201).json({
    ...newDriver,
