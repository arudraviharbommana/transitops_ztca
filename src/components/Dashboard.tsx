...
export default function Dashboard({
  ...
  const filteredActivityLogs = activityLogs.filter((log) => {
    const searchText = activitySearch.trim().toLowerCase();
    if (!searchText) return true;
    return log.user.toLowerCase().includes(searchText) || log.action.toLowerCase().includes(searchText);
  });
          ...
          <div className="p-4 flex-1 overflow-y-auto max-h-[350px] space-y-4">
            {filteredActivityLogs.length === 0 ? (
              <div className="text-center py-12 text-neutral-400 text-sm">
                No fleet activities matched the current filter.
              </div>
            ) : (
