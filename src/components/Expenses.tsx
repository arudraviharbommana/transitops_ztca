...
export default function Expenses({
  ...
  const filteredExpenses = expenses.filter(exp => {
    const searchText = expenseSearch.trim().toLowerCase();
    if (!searchText) return true;
    const vehicleText = getVehicleRegNumber(exp.vehicleId).toLowerCase();
    return exp.description.toLowerCase().includes(searchText) || vehicleText.includes(searchText) || exp.category.toLowerCase().includes(searchText);
  });

  const filteredFuelLogs = fuelLogs.filter(log => {
    const searchText = fuelSearch.trim().toLowerCase();
    if (!searchText) return true;
    const vehicleText = getVehicleRegNumber(log.vehicleId).toLowerCase();
    const driver = drivers.find(d => d.id === log.driverId);
    return vehicleText.includes(searchText) || (driver?.name || '').toLowerCase().includes(searchText) || (log.purpose || '').toLowerCase().includes(searchText);
  });
  ...
  return (
    <div id="expense-tab-panel" className="space-y-6">
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-4 shadow-sm">
        <input
          id={activeSubTab === 'expenses' ? 'expense-search-input' : 'fuel-search-input'}
          type="text"
          value={activeSubTab === 'expenses' ? expenseSearch : fuelSearch}
          onChange={(e) => activeSubTab === 'expenses' ? setExpenseSearch(e.target.value) : setFuelSearch(e.target.value)}
          placeholder={activeSubTab === 'expenses' ? 'Search expense by description, category, or vehicle...' : 'Search fuel by vehicle, driver, or purpose...'}
