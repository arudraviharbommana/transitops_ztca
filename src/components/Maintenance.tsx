...
export default function MaintenanceLogs({
  ...
  const [maintStatusFilter, setMaintStatusFilter] = useState<'All' | 'Active' | 'Closed'>('All');
  ...
  const filteredMaintenance = maintenance.filter(maint => {
    const vehicleInfo = getVehicleInfo(maint.vehicleId).toLowerCase();
    const description = maint.description.toLowerCase();
    const searchText = maintSearch.trim().toLowerCase();

    if (maintStatusFilter !== 'All' && maint.status !== maintStatusFilter) {
      return false;
    }
    ...
    return vehicleInfo.includes(searchText) || description.includes(searchText);
  });

  const activeMaintenance = filteredMaintenance.filter(m => m.status === 'Active');
  const closedMaintenance = filteredMaintenance.filter(m => m.status === 'Closed');
  ...
  return (
    <div id="maintenance-tab-panel" className="space-y-6">
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-4 shadow-sm flex flex-col md:flex-row md:items-center gap-3">
        <input
          id="maintenance-search-input"
          type="text"
          value={maintSearch}
          onChange={(e) => setMaintSearch(e.target.value)}
          placeholder="Search maintenance by vehicle or description..."
          className="w-full md:flex-1 px-3 py-2 border border-neutral-200 rounded-lg text-sm bg-neutral-50 focus:outline-none focus:ring-2 focus:ring-emerald-500"
        />
        <select
          id="maintenance-status-filter"
          value={maintStatusFilter}
          onChange={(e) => setMaintStatusFilter(e.target.value as 'All' | 'Active' | 'Closed')}
        ...
        <div className="bg-white border border-neutral-200 rounded-xl shadow-sm flex flex-col">
          <div className="p-4 border-b border-neutral-100 bg-amber-50/50 flex items-center justify-between">
            ...
            <span className="px-2 py-0.5 text-xs font-bold bg-amber-100 text-amber-800 rounded-full font-mono">
              {activeMaintenance.length} Units
            </span>
          </div>
        ...
        <div className="bg-white border border-neutral-200 rounded-xl shadow-sm flex flex-col">
          <div className="p-4 border-b border-neutral-100 bg-neutral-50 flex items-center justify-between">
            ...
            <span className="px-2 py-0.5 text-xs font-bold bg-neutral-200 text-neutral-700 rounded-full font-mono">
              {closedMaintenance.length} Logged
            </span>
          </div>
