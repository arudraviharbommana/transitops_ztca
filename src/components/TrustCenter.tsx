...
export default function TrustCenter({ currentUser, context }: TrustCenterProps) {
  ...
  const [reflection, setReflection] = useState(() => localStorage.getItem('transitops_trust_reflection') || '');
  ...
  const runSimulation = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/ztca/simulation/context-check', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          endpoint: '/api/trips',
          method: 'POST',
          actionName: 'Trust Center dispatch simulation',
          requiredPrivilege: 'DISPATCH_TRIP'
        })
      });
      setSimulation(await response.json());
    } catch (error) {
      setSimulation({ error: 'Simulation service unavailable.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveReflection = () => {
    localStorage.setItem('transitops_trust_reflection', reflection);
    setSavedAt(new Date().toLocaleTimeString());
  };

  const riskScore = simulation?.risk?.totalScore ?? 0;
  const outcome = simulation?.decision?.outcome || 'RUN SIMULATION';
  ...
  return (
    <div className="space-y-6">
      <section className="bg-neutral-900 text-white rounded-xl p-6 border border-neutral-800 shadow-lg">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
          <div className="max-w-2xl">
            ...
            <p className="text-neutral-300 mt-2 text-sm leading-6">A working reference for role boundaries, seven zero-trust policy decisions, contextual risk scoring, saved reflections, and the limits of this prototype.</p>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <section className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3 mb-4"><div><h2 className="text-lg font-black text-neutral-900 flex items-center gap-2"><Gauge className="w-5 h-5 text-amber-600" /> Rule-based risk detection</h2><p className="text-xs text-neutral-500 mt-1">Scores add context signals and cap at 100.</p></div><button onClick={runSimulation} disabled={loading} className="px-3 py-2 rounded-lg bg-neutral-900 text-white text-xs font-bold flex items-center gap-2 disabled:opacity-50"><CircleDot className="w-4 h-4" /> {loading ? 'Checking...' : 'Run live check'}</button></div>
          ...
          <div className="mt-4 rounded-lg bg-neutral-900 text-white p-4 flex items-center justify-between"><div><div className="text-[10px] uppercase text-neutral-400 font-bold">Current simulation</div><div className="text-sm font-bold mt-1">{simulation?.error || outcome}</div></div><div className="text-right"><div className="text-3xl font-black text-emerald-400">{riskScore}<span className="text-sm text-neutral-400">/100</span></div><div className="text-[10px] font-black text-neutral-400">{simulation ? scoreLabel(riskScore) : 'NOT RUN'}</div></div></div>
          {simulation?.decision?.reason && <div className="text-xs text-neutral-500 mt-3"><b>Decision rationale:</b> {simulation.decision.reason}</div>}
        </section>

        <section className="bg-white border border-neutral-200 rounded-xl p-5 shadow-sm"><h2 className="text-lg font-black text-neutral-900 flex items-center gap-2"><Database className="w-5 h-5 text-indigo-600" /> Data saving & reflection</h2><p className="text-xs text-neutral-500 mt-1 mb-4">Operational records and audit logs are saved as JSON by the backend repositories. This reflection is saved locally for this browser.</p><textarea value={reflection} onChange={event => setReflection(event.target.value)} placeholder="Record what this prototype proves, where evidence is strong, and what still needs production hardening..." className="w-full min-h-[145px] border border-neutral-300 rounded-lg p-3 text-sm resize-y focus:outline-none focus:ring-2 focus:ring-emerald-500" /><div className="flex items-center justify-between mt-3"><span className="text-[11px] text-neutral-500">{savedAt ? `Saved locally at ${savedAt}` : 'Not saved in this browser yet'}</span><button onClick={saveReflection} className="px-3 py-2 rounded-lg bg-emerald-600 text-white text-xs font-bold flex items-center gap-2"><Save className="w-4 h-4" /> Save reflection</button></div><div className="mt-5 border-t border-neutral-200 pt-4"><h3 className="font-black text-sm flex items-center gap-2"><ClipboardCheck className="w-4 h-4 text-neutral-700" /> Known limitations</h3><ul className="mt-2 text-xs text-neutral-600 space-y-1.5 list-disc pl-4"><li>Passwords and the Step-Up demo PIN are not production-grade authentication.</li><li>JSON files are local persistence, without transactions, encryption, backups, or multi-instance consistency.</li><li>Some role boundaries remain UI/documentation-only and need a centralized server-side authorization matrix.</li><li>Risk signals are deterministic heuristics; there is no behavioral baseline, device attestation, or real-time revocation.</li><li>Simulation logs are explicitly marked as simulations and should not be treated as operational evidence.</li></ul></div></section>
      </div>
    </div>
  );
}