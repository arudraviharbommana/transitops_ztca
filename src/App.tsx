...
export default function App() {
  ...
  const handleAddDriver = async (driver: Omit<Driver, 'id'>): Promise<boolean> => {
    try {
      ...
      if (res.ok) {
        const loginDetails = apiPayload?.loginEmail && apiPayload?.temporaryPassword
          ? ` Login enabled: ${apiPayload.loginEmail} / ${apiPayload.temporaryPassword}`
          : '';
        triggerToast(`Driver profile registered: ${driver.name}.${loginDetails}`, 'success');
        fetchAllData();
        return true;
      }

      const apiError = apiPayload?.error || 'Failed to register driver.';
